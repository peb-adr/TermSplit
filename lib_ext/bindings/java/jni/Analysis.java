...
public class Analysis extends AnalysisRefMut implements AutoCloseable {
    ...
    public static TimeSpan calculateSumOfBest(RunRef run, boolean simpleCalculation, boolean useCurrentRun, byte method) {
        ...
        TimeSpan result = new TimeSpan(LiveSplitCoreNative.Analysis_calculateSumOfBest(run.ptr, simpleCalculation, useCurrentRun, method));
        if (result.ptr == 0) {
            return null;
        }
        return result;
    ...
    public static TimeSpan calculateTotalPlaytimeForRun(RunRef run) {
        ...
        TimeSpan result = new TimeSpan(LiveSplitCoreNative.Analysis_calculateTotalPlaytimeForRun(run.ptr));
        return result;
    }
    /**
    ...
    public static TimeSpan calculateTotalPlaytimeForTimer(TimerRef timer) {
        ...
        TimeSpan result = new TimeSpan(LiveSplitCoreNative.Analysis_calculateTotalPlaytimeForTimer(timer.ptr));
        return result;
    }
    Analysis(long ptr) {
        super(ptr);
